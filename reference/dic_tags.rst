...
.. note::
    ...
    When using the ``auto_alias`` tag, it is not mandatory to define the aliased
...
plain inlined ``include_once``. The benef it is a complete bypass of the autoloader
...
Add this tag to a service and its class will not be preloaded when using
...
metadata and Doctrine metadata (if you are using Doctrine) or Propel metadata
(if you are using Propel).
...
when handling the request, if it was not done by one of the commands yet.
...
The ``isOptional()`` method should return true if it is possible to use the
...
.. caution::
    ...
    will not try to execute it again for the next requests. Therefore, your
...
If you are loading translations from a database, you will still need a resource
...
configuration and tag it with ``twig.extension``. If you are using the
...
regular services but they need to be tagged with ``twig.runtime``. If you are using the
:ref:`default services.yaml configuration <service-container-services-load-example>`,
the service is auto-registered and auto-tagged. But, you can also register it manually:
...
to perform some sort of action on an object right before it is validated.
For example, it is used by Doctrine to query for all of the lazily-loaded
data on an object before it is validated. Without this, some data on a Doctrine
