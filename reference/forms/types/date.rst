...
The underlying data used for this field type can be a ``DateTime`` object, a
string, a timestamp or an array. As long as the ``input`` option is set
correctly, the field will take care of all of the details (see the ``input``
option).
...
``DateTime`` object. The following configures the ``date`` type for that field
...
data. For example, if the ``publishedAt`` field's data were a unix timestamp,
