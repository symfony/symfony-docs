...
.. note::
    ...
    Symfony2 also contains a method for cache *busting*, where the final URL
...
because an internal Symfony2 controller opens the files and serves back the
...
by Symfony2 (as the asset files are in the ``dev`` environment). This is on
purpose - letting Symfony2 generate these files dynamically in a production
...
First, tell Symfony2 to stop trying to process these files dynamically. Make
...
Next, since Symfony2 is no longer generating these assets for you, you'll
