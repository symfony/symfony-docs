...
#. A :ref:`gateway cache<book-cache-gateway>`, or reverse proxy, is
...
#. :ref:`HTTP cache<book-cache-http-intro>` headers are used
...
#. HTTP :ref:`expiration and validation<book-cache-expiration-validation>`
...
#. :ref:`Edge Side Includes<book-cache-edge-side-includes>` (ESI) allow HTTP
...
(See :ref:`book-cache-http-intro`). If the same resource is requested again,
...
.. note::
    ...
    :ref:`book-cache-expiration-validation` section.
...
In the :ref:`book-cache-optimizing-validation` section, you'll see how validation
...
This is done via the ``render`` helper (See :ref:`book-templating-embedding-controller`
for more details).
...
.. tip::
    ...
    :doc:`Security Chapter </book/security>` for more information
    on how to do this.
