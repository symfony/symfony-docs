...
Besides the syntax, the linter also checks the contents of the files against a
`JSON Schema`_. Install the library used to perform that validation first:
...
.. code-block:: terminal
    ...
    # validate every file against the schema it declares itself
...
A file declares the schema it must conform to in a comment placed before any
YAML content. Editors supporting the `YAML language server`_ read the same
header, so they validate the file as you type:
...
The shorter ``# $schema=path/to/schema.json`` header is supported too, and
relative locations are resolved from the directory of the file being linted.
Files that declare no schema are reported as valid, so you can run the command
on an entire directory.

In a Symfony application, the ``lint:yaml`` command also applies the schemas of
the files following the Symfony conventions, without any header:

.. code-block:: terminal

    $ php bin/console lint:yaml config/ --check-schema

The files in ``config/packages/`` are validated against the ``config/schema.json``
file generated by Symfony, which describes the options of every installed bundle.
The routing, service, serializer and validator configuration files are validated
against the schema of their own component, wherever they are stored (including
inside bundles).
