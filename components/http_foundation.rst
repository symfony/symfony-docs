...
The list of private subnets is defined in the ``PRIVATE_SUBNETS`` constant of
the :class:`Symfony\\Component\\HttpFoundation\\IpUtils` class. It's the same
list used by the ``private_ranges`` value of the :doc:`trusted_proxies option </deployment/proxies>`:
