...
**UUID v1**
    Generates the UUID using the MAC address of your device and a timestamp
...
**UUID v3**
    ...
    This variant is useful to generate UUIDs from arbitrary strings. It works by
    populating the UUID contents with the``md5`` hash of concatenating the namespace
    and the name::
...
**UUID v4**
    ...
    so you don't have to pass any constructor argument::
...
**UUID v5**
    ...
    ``md5`` to hash the given namespace and name
    (`read UUIDv5 spec <https://datatracker.ietf.org/doc/html/draft-ietf-uuidrev-rfc4122bis#name-uuid-version-5>`__).

**UUID v6**
    It's lexicographically sortable (like :ref:`ULIDs <ulid>`) and contains a 60-bit timestamp and 63 extra unique bits
...
**UUID v7**
    Generates time-ordered UUIDs based on the Unix Epoch timestamp source (the number of
