...
* :doc:`Install it via Composer</components/using_components>` (`symfony/http-kernel`_ on Packagist).
...
.. image:: /images/components/http_kernel/kernel-interface-code.png
   :align: center

Internally, :method:`HttpKernel::handle<Symfony\\Component\\HttpKernel\\HttpKernel::handle>` -
the concrete class that implements :class:`Symfony\\Component\\HttpKernel\\HttpKernelInterface` -
...
The ``HttpKernel::handle()`` method works internally by throwing events.
...
Initially, using the ``HttpKernel`` is really simple, and involves creating
an :doc:`event dispatcher</components/event_dispatcher/introduction>` and
a :ref:`controller resolver<component-http-kernel-resolve-controller>` (explained
...
to the events discussed below::
    ...
    // by throwing events, calling a controller, and returning the response
...
parts of the system, or return a Response if possible (e.g. a security layer
that denies access)

:ref:`Kernel Events Information Table<component-http-kernel-event-table>`

The first event that is thrown inside :method:`HttpKernel::handle<Symfony\\Component\\HttpKernel\\HttpKernel::handle>`
...
Listeners to this event can be quite varied. Some listeners - such as a security
...
If a Response is returned at this stage, the process skips directly to the
:ref:`kernel.response<component-http-kernel-kernel-response>` event.
...
return a Response directly, or to add information to the Request (e.g. setting
the locale or setting some other information on the Request attributes).
...
.. image:: /images/components/http_kernel/05-controller-resolver-code.png
   :align: center
...
will be called after another event - ``kernel.controller`` - is thrown.
...
throws the ``kernel.controller`` event. Listeners to this event might initialize
...
Usually, the controller will return a Response object. If this is true, then
the work of the kernel is just about done! In this case, the next step is
...
If the controller doesn't return a Response object, then the kernel throws
...
.. sidebar:: ``kernel.view`` in the Symfony Framework
    ...
    event. However, one core bundle - :doc:`SensioFrameworkExtraBundle </bundles/SensioFrameworkExtraBundle/index>` -
...
is thrown directly afterwards. A typical listener to this event will modify
the Response object in some way, such as modifying headers, adding cookies,
or even changing the content of the Response itself (e.g. injecting some JavaScript
before the end `</body>` tag of an HTML response).

After this event is thrown, the final Response object is returned from
:method:`Symfony\\Component\\HttpKernel\\HttpKernel::handle`. In the most
typical use-case, you can then call the :method:`Symfony\\Component\\HttpFoundation\\Response::send`
...
``kernel.exception`` event is thrown so that your system can somehow respond
to the exception.
...
thrown during the ``HttpKernel::handle`` cycle. Typically a listener is a PHP
...
.. _symfony/http-kernel: https://packagist.org/packages/symfony/http-kernel
