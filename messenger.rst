...
.. tip::
    ...
    Run ``php bin/console debug:messenger`` to see the transports each message
    is routed to, or pass a class to its ``--message`` option to inspect a
    single message.
...
option to handle several messages in parallel:

.. code-block:: terminal

    $ php bin/console messenger:consume async --concurrency=4

Messages are handled by a pool of PHP processes spawned from the application
console, each of them handling one message at a time. This requires the
...
.. note::
    ...
    Messages handled by a :ref:`batch handler <messenger-handler-batch>` are
    pinned to a single process per bus, transport and message class, so they
    are still handled one after the other.

.. versionadded:: 8.2

    The ``--concurrency`` option was introduced in Symfony 8.2.

By default, the worker fetches as many messages per iteration from the transport
as the value of ``--concurrency``. Use the ``--fetch-size`` option to fetch a
different number of messages per iteration, reducing the number of round trips
to the transport:
...
Running messages in parallel is already covered by
:class:`Symfony\\Component\\Messenger\\Execution\\ParallelExecutionStrategy`,
which the :ref:`--concurrency option <messenger-concurrency>` uses. To customize
how messages are processed in some other way, implement
...
``delay[granularity]`` (default: two significant digits)
    Each delayed message (delayed with a ``DelayStamp`` or by the retry
    mechanism) is sent to a delay queue whose name and ``x-message-ttl``
    argument include the delay, so each different delay creates a new queue.
    When delays are randomized (e.g. with the ``jitter`` option of the retry
    strategy), this can create a large number of queues.

    To limit this, delays are rounded up to a multiple of this number of
    milliseconds, so messages are released a bit later than requested. By
    default, delays are rounded up to two significant digits (e.g. ``5234``
    becomes ``5300`` and ``52345`` becomes ``53000``). This means that a
    message is never released more than 10% later than requested, regardless
    of the length of the delay. Set this option to ``1`` to disable rounding
    and use the delays as they are.
...
object using the :doc:`Validator component </validation>` before handling it.
...
along with the transports each message is routed to. You can also restrict
the list to a specific bus by providing its name as an argument.
...
Routing is configured once for the whole application, so the ``Transports``
section is displayed once, after the buses. Rules defined with the
``#[AsMessage]`` attribute are marked as such (the
:ref:`configuration takes precedence <messenger-routing>` over them). The
:ref:`failure transport <messenger-failure-transport>` of each transport is
shown, except when a transport is its own failure transport.

Pass a message class to the ``--message`` option to see what happens when that
class is dispatched, even if the class is not registered as a service. The
class is resolved the way ``dispatch()`` resolves it, so handlers and rules
defined for a parent class, an interface or a namespace apply too. The output
only includes what applies to that message, and says ``not handled`` or
``not routed`` when nothing does:
