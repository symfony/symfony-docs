...
Add some ``Preloader::ignore()`` calls to your ``config/preload.php`` file to
exclude them. As the ``Preloader`` class is only autoloadable once the Composer
autoloader is loaded, require it first::
    ...
    // this is the contents generated by the Symfony Flex recipe; the name of the
    // file depends on your kernel class (e.g. 'srcApp_KernelProdContainer.preload.php')
