...
Preloading also follows the types used in the public properties, the method
arguments and the return types of each preloaded class. A service that
references a large third-party SDK can pull thousands of classes into
preloading this way. The ``container.no_preload`` tag doesn't help here,
because those classes are not services. Call ``Preloader::ignore()`` in your
``config/preload.php`` file to exclude them::

    // config/preload.php
    use Symfony\Component\DependencyInjection\Dumper\Preloader;

    // the Preloader class needs the Composer autoloader
    require_once dirname(__DIR__).'/vendor/autoload.php';

    // ignore a single class...
    Preloader::ignore('Microsoft\Graph\GraphServiceClient');
    // ...or a whole namespace (the name must end with a backslash)
    Preloader::ignore('Microsoft\\Graph\\');

    // this is the code added by the Symfony Flex recipe; call ignore() before it
