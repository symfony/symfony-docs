...
understand because developers cannot reproduce them in their computers. That's
when  we will ask you to create a "bug reproducer", which is the minimum amount of
...
Symfony framework, it is enough to share a small PHP script that when executed
shows the bug::
...
If the bug is related to the Symfony Framework or if it is too complex to create
a PHP script, it is better to reproduce the bug by creating a new project. To do so:
...
#. Now you must add the minimum amount of code to reproduce the bug. This is the
   trickiest part and it is explained a bit more later.
...
#. If you can see the bug, you are done and you can already share the code with us.
#. If you cannot see the bug, you must keep making small changes. For example, if
