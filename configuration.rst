...
* The ``reference.php`` file is autogenerated when the container is compiled in
  debug mode; it contains PHP definitions that improve IDE autocompletion for
  configuration written in PHP;
* The ``schema.json`` file is autogenerated when the container is compiled in
  debug mode and the ``symfony/yaml`` package is installed; it merges the
  configuration trees of every bundle so editors can validate and autocomplete
  YAML files;
...
When the container is compiled in debug mode, Symfony exports the configuration
options of every installed bundle into ``config/reference.php`` and
``config/schema.json`` (see
:ref:`Generate a Configuration Reference <bundle-friendly-config-reference>`).
The following sections explain how to make your editor use them.

Autocompletion for PHP Configuration
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

``config/reference.php`` exposes PHP array-shapes, so IDEs and static analysers
such as PhpStorm, PHPStan or Psalm autocomplete and validate configuration
written with the ``App::config()`` helper without any extra setup, as long as
the file is committed and part of your project.

Autocompletion for YAML Configuration
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Editors do not detect ``config/schema.json`` automatically, so you have to map
it to the YAML files stored under ``config/`` once:

* **Visual Studio Code**, **Neovim** and any editor backed by
  `yaml-language-server`_: add a modeline comment at the top of the YAML file:
  ...
  ``config/packages/<env>/``. In Visual Studio Code you can also declare the
  mapping globally in ``.vscode/settings.json``:
...
* **PhpStorm**: open ``Settings > Languages & Frameworks > Schemas and DTDs >
  JSON Schema Mappings``, add a new mapping pointing the schema file
  ``config/schema.json`` to the file path pattern ``config/**/*.yaml``.
