...
* The ``schema.json`` file is autogenerated by Symfony and describes the options of
  every installed bundle as a `JSON Schema`_, which IDEs can use to provide
  autocompletion and validation when using YAML as the configuration format;
...
Both ``reference.php`` and ``schema.json`` are written while compiling the
container in debug mode, so they follow the bundles installed in your
application. Generating ``schema.json`` also requires the YAML component.

.. versionadded:: 8.2

    The ``schema.json`` file was introduced in Symfony 8.2.
...
* **YAML**: simple, clean and readable, and IDEs can provide autocompletion and
  validation for it thanks to the generated ``config/schema.json`` file.
  :doc:`Learn the YAML syntax </reference/formats/yaml>`;
