...
In this case, you will need a ``Category`` class, and a way to relate a
...
command, which will ask you several questions about your relationship. If you are
...
.. sidebar:: What is the ArrayCollection Stuff?
    ...
    *exactly* like an array, but has some added flexibility. Just imagine that
    it is an ``array`` and you will be in good shape.
...
Now you can see this new code in action! Imagine you are inside a controller::
...
If you are new to an ORM, this is the *hardest* concept: you need to stop thinking
...
category, but the category data is not actually retrieved until you ask for
the category (i.e.it is "lazily loaded").
...
.. sidebar:: Relationships and Proxy Classes
    ...
    You will probably never even notice that your ``$category`` object is actually
    a proxy object.
...
If you know up front that you will need to access both objects, you
...
So far, you have updated the relationship by calling ``$product->setCategory($category)``.
...
Does this means it is not possible to call ``$category->addProduct()`` or
...
.. note::
    ...
    If you are using annotations, you will need to prepend all annotations with
